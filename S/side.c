 ...
 * version 0.1 by Clement Vasseur <nitro@epita.fr>
 *
 * Adaptated for use with PedroM by PpHd (ppelissi@caramail.com)
 ...
 *
 * Bug: 
 *	Cut/Copy/Paste in AMS version
 */
//#define PEDROM // Mustn't be compiled with regparm !
...
#define OSResetTimer OSTimerRestart
#define	STAT_STATE	KEY_STATUS
#define STAT_2ND	0x1000
#define STAT_DIAMOND	0x2000
#define	STAT_SHIFT	0x4000

#endif

static char TextFont46[] = {
...
0x07,0x07,0x07,0x07,0x07,0x00,0x00,0x02,0x05,0x00,
...
enum { INPUT_NUM, INPUT_PATH, INPUT_ALL };
enum { ERR_NONE, ERR_MEMORY, ERR_FILE };
enum { EXEC_OTH, EXEC_SC, EXEC_AS };
enum { C_WHITE, C_BLACK, C_INVERSE = 128 };
enum { I_NONE, I_SECOND, I_DIAMOND, I_SHIFT, I_ALPHA, I_ALOCK, I_CAPS };
enum { K_SECOND = 4096, K_DIAMOND = 8192, K_SHIFT = 16384 };
#ifdef	TI92P
enum { K_UP = 338, K_DOWN = 344, K_RIGHT = 340, K_LEFT = 337, K_F1 = 268,
...
#else
#ifdef	TI89
enum { K_UP = 337, K_DOWN = 340, K_RIGHT = 344, K_LEFT = 338, K_F1 = 268,
       K_F2 = 269, K_F3 = 270, K_F4 = 271, K_F5 = 272, K_F6 = 273, K_F7 = 274,
       K_F8 = 275, K_ESC = 264, K_QUIT = 4360, K_APPS = 265, K_SWITCH = 4361,
       K_MODE = 266, K_BACKSPACE = 257, K_INS = 4353, K_CLEAR = 263,
       K_VARLNK = 4141, K_CHAR = 4139, K_ENTER = 13, K_STO = 258, K_ON = 267,
...
static void draw_hline(short x1, short x2, short y, short color)
{
...
static void draw_vline(short x, short y1, short y2, short color)
{
...
static void draw_char46(short a)
{
  register long *addr = (long *) ((char *) LCD_MEM + penRow*30 + (((penCol-4) >>3) & 0xfffe));
  register unsigned short cnt  = 24-((penCol-4)&15), h = 6-1;
  register long mask = ~(0x0FL << cnt);
  register unsigned char *sprite = TextFont46 + (a - ' ') * 6;
  if (penCol == 0)
...
{ if (stream_ptr == curs_ptr) stream_ptr = gap_ptr; \
  ...
  if (c && c < 32 && c != CR && c != EOF) c = 127; \
  if (c && c != EOF) stream_ptr++; \
}

static void draw_text_screen(short nb_row, short nb_col, void *stream)
{
  BOOL end_of_line;
  short c1, inv, c2, cnt, i;
  unsigned char *scr_line = LCD_MEM + 30 * 8;
  unsigned char *spr1, *spr2, *scr_addr, *addr;
  char *sel_low, *sel_hi;
  register char *stream_ptr = stream;
  ...
  while (nb_row--)
  {
    ...
    for (cnt = 0; cnt < nb_col; cnt+=2)
    {
      if (end_of_line)
       ...
       {
        inv = ( (sel_low <= stream_ptr) && (stream_ptr < sel_hi) ? 1 : 0);
        read_char(c1);
        if (c1 == CR || c1 == EOF)
         {
          end_of_line = TRUE;
         ...
         {
         inv |= ( (sel_low <= stream_ptr) && (stream_ptr < sel_hi) ? 2 : 0);
         read_char(c2);
         if (c2 == CR || c2 == EOF)
          {
           end_of_line = TRUE;
...
static void draw_str46(const char *str)
{
  while (*str)
    draw_char46(*str++);
}

static void draw_str_center46(const char *str)
{
...
static void print_str(const char *format, ...)
{
  /*char buffer[512];
  va_list arglist;
  ...
  va_list a;
  va_start(a, format);
  vcbprintf((int (*)(short, FILE *)) draw_char46, NULL, format, a);
  va_end(a);
}

static void prepare_bottom(void)
{
...
{
  short i;
  static char *menu[] = { "Build", "Exec", "Goto", "Find", "Replace", "Config", "About" };
...
static error_t error(error_t state)
{
  prepare_bottom();
  if (state == ERR_MEMORY)
...
static BOOL prompt(const char *str)
{
  prepare_bottom();
...
static BOOL input_str(char *buf, short maxlen, short mode)
{
  ...
  do
  {
	...
	(isdigit(key) || (mode == INPUT_ALL && 32 <= key && key<=127) ||
	 (mode == INPUT_PATH && (isalnum(key) || key == '_' || key == '\\'))))
      buf[i++] = (mode == INPUT_PATH) ? tolower(key) : key;
  } while (((key != K_ENTER) || !i) && (key != K_ESC));
  return key == K_ENTER;
}

static BOOL input_num(const char *prompt, int *num)
{
  char buf[5];
  BOOL ret;
...
static void about(void)
{
  ...
  draw_str_center46("v0.1 by Clem Vasseur");
  draw_str_center46("<nitro@epita.fr>");
  draw_str_center46("Adapted by PpHd for PedroM");
  draw_str_center46("<ppelissi@caramail.com>");
  get_key();
  draw_menu_bar();
}

static void draw_str_config(const char *str1, short mode, void *val)
{
  int col = penCol;
  short i;
  ...
  switch (mode)
  {
    case 0: draw_str46(*(BOOL *) val ? "Yes" : "No "); break;
    case 1: print_str("%3d", *(short *) val); break;
    default:
      draw_str46((char *) val);
...
static void config(void)
{
  ...
  show_msg("[UP]/[DOWN]=Move  [LEFT]/[RIGHT]=Change");
  window(150+8, 54+6);
  row = penRow + 2;
  col = penCol;
  ...
  do {
    ...
    switch (key = get_key())
    {
      case K_RIGHT: mod = 1; break;
      case K_LEFT: mod = -1; break;
      case K_UP: if (cursor) cursor--; break;
      case K_DOWN: if (cursor < 6) cursor++;
    }
    if (mod)
    {
      switch (cursor)
	...
	case 2:
          ...
          break;
        case 3: auto_code ^= 1; break;
        case 4: auto_indent ^= 1; break;
        case 5:
...
static void forget_selection(void)
{
  if (select != 0)
	{
	select = 0;
...
static void move_right_word(void)
{
...
static void move_left_word(void)
{
  do {
    move_left();
...
static error_t write(short c)
{
  ...
  if (curs_ptr + 2 >= gap_ptr)
  {
    ...
    if ((ptr = realloc(file_ptr, end_ptr - file_ptr + 1 + BUF_SIZE)))
    {
      ...
      gap_ptr += ptr_diff;
      end_ptr += ptr_diff;
      gap_ptr = memmove(gap_ptr + BUF_SIZE, gap_ptr, end_ptr - gap_ptr + 1);
      end_ptr += BUF_SIZE;
    }
    else
...
static void select_proc(void (*proc)(void))
{
  short sel = select;
  char *ptr = curs_ptr;
...
static error_t clipboard_copy(void)
{ 
  short cnt;
  HANDLE n_clipboard;
...
static void find(void)
{
  ...
  if (input_str(find_str, 31, INPUT_ALL))
    do move_right();
    while (*gap_ptr != EOF && strncmp(gap_ptr, find_str, strlen(find_str)));
  draw_menu_bar();
}
  ...
  if (input_str(find_str, 31, INPUT_ALL))
    ...
    if (input_str(replace_str, 31, INPUT_ALL))
	    ...
	    while (*gap_ptr != EOF && doit)
			...
			show_msg("Replace [ENTER], Next [SPACE], All [A], Cancel [ESC]");
			if (doit == 1)
				key = get_key();
			else	key = K_ENTER;
			switch (key)
...
static void chars_table(void)
{
  short i, j, key;
  short c_col = 0, c_row = 0;
  short col, row;

  show_msg("[ARROWS]=Move  [ENTER]=Copy  [ESC]=Exit");
  window(16 * 5 + 3, 6 * 8 + 2);
  col = penCol;
  row = penRow;
  ...
  do {
    ...
    switch (key = get_key())
    {
      case K_RIGHT: c_col = (c_col + 1) & 15; break;
      case K_LEFT: c_col = (c_col - 1) & 15; break;
      case K_UP: c_row = ((--c_row) < 0 ? 5 : c_row); break;
      case K_DOWN: c_row = (c_row + 1) % 6; break;
      case K_ENTER: write(' ' + c_row * 16 + c_col);
    }
  } while (key != K_ENTER && key != K_ESC);
  draw_menu_bar();
}
...
static void show_text2(void)
{
  char *ptr;
  short key;
  char	buff_name[20];
  SYM_ENTRY *sym = SymFindPtr(StrToTokN(text2_name, buff_name),0);
  
  if (sym == NULL)
    return;
  show_msg("[UP]/[DOWN]=Scroll  [ESC]or[APPS]=Exit");
  ptr = HeapDeref(sym->handle);
  do {
    draw_text_screen((LCD_HEIGHT - 16) / 7, LCD_WIDTH / 4, ptr + text2_pos);
    key = get_key();
    if (key == K_DOWN)
    {
      key = LCD_WIDTH / 4;
...
void run_side(void)
{
  ...
  if (filename) strcpy(filebuffer,filename);
  else filebuffer[0] = '\0';
  filename = filebuffer;
  ...
  do {
    ...
    do {
      ...
      key = get_key();
      cursor_on = FALSE;
      if (32 <= key && key <= 127) write(key);
      else switch (key)
      {
      	case K_COPY: clipboard_copy(); break;
      	case K_PASTE: clipboard_paste(); break;
        case K_CUT: if (clipboard_copy() == ERR_NONE) clear_selection(); break;
        case K_LEFT: move_left(); break;
        case K_RIGHT: move_right(); break;
        case K_UP: move_up(); break;
        case K_DOWN: move_down(); break;
        case K_SECOND+K_LEFT: line_start(); break;
        case K_SECOND+K_RIGHT: line_end(); break;
        case K_SECOND+K_UP: page_up(); break;
        case K_SECOND+K_DOWN: page_down(); break;
        case K_SHIFT+K_LEFT: select_left(); break;
        case K_SHIFT+K_RIGHT: select_right(); break;
        case K_SHIFT+K_UP: select_up(); break;
        case K_SHIFT+K_DOWN: select_down(); break;
        case K_DIAMOND+K_UP: page_top(); break;
        case K_DIAMOND+K_DOWN: page_end(); break;
        case K_DIAMOND+K_LEFT: move_left_word(); break;
        case K_DIAMOND+K_RIGHT: move_right_word(); break;
        case K_BACKSPACE: back(); break;
        case K_DIAMOND+K_BACKSPACE: del(); break;
        case K_ENTER: write_cr(); break;
        case K_CLEAR: clear_line(); break;
        case K_CHAR: chars_table(); break;
        case K_APPS: show_text2(); break;
        case K_F1: compile(exec_name); break;
        case K_F2: compile(exec_name2); break;
        case K_F3: go_line(); break;
        case K_F4: find(); break;
        case K_F5: replace(); break;
        case K_F6: config(); break;
        case K_F7: about();
      }
