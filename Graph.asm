...
; SideEffect: Do not destroy a0/a1
PortSet:
...
MakeWinRect:
	move.w	4(a7),WIN_RECT_X1
	move.w	6(a7),WIN_RECT_Y1
	move.w	8(a7),WIN_RECT_X2
	move.w	10(a7),WIN_RECT_Y2
	lea	WIN_RECT_X1,a0
	rts
...
ScrRectShift:
	...
	pea	(a4)				; Rect
	pea	(a3)				; Clip
	bsr	ScrRectOverlap			; Intersection of the 2 rect
	tst.w	d0
...
\O2	
	...
	pea	(a2)				; Source
	pea	4(a2)				; Dest
	bsr	ScrRectOverlap			; Intersection of the 2 rect (MUST Overlap)
	; Get the region dest
	bsr	\ReadRect	; Read Rect (d0, d1, d2, d3)
	bsr	BitmapNew	
	move.w	d0,d7		; Handle
...
ScreenClear:
	...
	addq.w	#1,d0			; I am sure some people will set a not-even screen.
	mulu.w	CURRENT_INCY,d0		; d0 = Size to erase
	subq.l	#1,d0			; d0 = Size((SizeY+1)*IncY)-1
\loop		move.b	d2,(a0)+	; So a byte fill only :(
		dbf	d0,\loop
	rts
...
ST_helpMsg_reg:
	...
	bsr	ScrRectFill
	lea	FullRect(pc),a0
	bsr	SetCurClip_reg	; Set clipping coordinate
	...
	beq.s	\End
		...
		bsr	DrawStr		; DrawStr
		move.w	d4,(a7)
		bsr	FontSetSys	; Restore old font		
		st.b	HELP_BEING_DISPLAYED
\End:	lea	CLIP_TEMP_RECT,a0
	bsr	SetCurClip_reg		; Restore current clipping
	move.l	a6,a7
...
ST_eraseHelp:
	...
	beq.s	\quit
		...
		suba.l	a0,a0
		bsr	ST_helpMsg_reg	; Erase the current context
...
\done:		; Display the string
		...
		bsr	DrawStr				; DrawStr
		lea	10(a7),a7
\end	bsr	FontSetSys	; Restore old font		
	addq.l	#2,a7
	rts
...
\Ok		pea	ST_batt_str(pc)
		...
		bsr	DrawStr		; DrawStr
		lea	10(a7),a7
\done	bsr	FontSetSys	; Restore old font		
	addq.l	#2,a7
	rts
...
ST_refDsp:
	...
	bhi.s	\error
		add.w	d0,d0
		move.w	ST_refDsp_Table(pc,d0.w),d0
		lea	ST_refDsp_Table(pc,d0.w),a0
		bsr	ST_helpMsg_reg
\error	rts
	
ST_refDsp_Table:
	dc.w	ST_Str1-ST_refDsp_Table
	dc.w	ST_Str2-ST_refDsp_Table
	dc.w	ST_Str3-ST_refDsp_Table
	dc.w	ST_Str4-ST_refDsp_Table
	dc.w	ST_Str5-ST_refDsp_Table
	dc.w	ST_Str6-ST_refDsp_Table
	dc.w	ST_Str7-ST_refDsp_Table
	dc.w	ST_Str8-ST_refDsp_Table
	dc.w	ST_Str9-ST_refDsp_Table
	dc.w	ST_StrA-ST_refDsp_Table
	dc.w	ST_StrB-ST_refDsp_Table
	dc.w	ST_StrC-ST_refDsp_Table
	dc.w	ST_StrD-ST_refDsp_Table
...
DrawClipRect:
	...
	bsr	SetCurClip_reg		; Set it as current
